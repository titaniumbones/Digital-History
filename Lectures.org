...
#+begin_slideblock
*For most of us, this is experimental.*
- no canon
...
#+ATTR_REVEAL: :frag (appear) 
#+begin_slideblock
*Systematic inquiry won't get us there*
...
- Aims:
  ...
  - plan your own professional or independent fuutre as a digital historian
#+end_slideblock
*** Objectives
:PROPERTIES:
...
#+begin_paired
[[/home/matt/DH/images/dish-spoon-territory.png]]
#+end_paired
#+begin_paired
...
:END:
#+INCLUDE: ./His393Syllabus.org::assignments_table
...
- Zero to Blog Post
- A Feast of Ministers
- @@html:<span class="urgent">@@DOM and Data@@html:</span>@@
- Oral History
#+end_slideblock
#+ATTR_REVEAL: :frag (appear) 
#+begin_slideblock
...
- A Feast of Ministers
- DOM and Data
- @@html:<span class="urgent">@@Spatial History@@html:</span>@@
- Oral History
#+end_slideblock
#+ATTR_REVEAL: :frag (appear) 
#+begin_slideblock
...
- Spatial History
- Oral History
- @@html:<span class="urgent">@@Project Proposal@@html:</span>@@
- @@html:<span class="urgent">@@All Reflection Questions@@html:</span>@@
#+end_slideblock
...
** Before next class
- [[https://github.com/join?source=header-home][Get yourself a Github user account]]
- [[https://classroom.github.com/classrooms/54332214-digital-history-fall-2019][Join the "Github Classroom"]]
...
:PROPERTIES:
:EXPORT_FILE_NAME: 01-02-git
:END:
** COMMENT Network Diagram Code for this Lecture

*** ACTION Setup Plantuml (move to init!!)
 #+begin_src emacs-lisp
...
#+begin_src plantuml :file help.svg
help skinparams
#+end_src
...
*** Git Push
#+NAME: push-start
#+begin_src plantuml :file images/git-push0.svg
<<init-uml>>
skinparam ArrowThickness 8
skinparam ArrowColor Black
skinparam sequenceParticipantBackgroundColor Green
skinparam sequenceActorBackgroundColor Green

title initial state - no local development

together {
DEV_GIT_COMMIT(1, "1 \n initial commit") 
DEV_GIT_COMMIT(2, "2 \n origin/master") #LightBlue
}
...
#+RESULTS: push-start
[[file:images/git-push0.svg]]

#+begin_src plantuml :file images/git-push1.svg
<<push-start>>
title after git clone/pull
...
#+RESULTS:
[[file:images/git-push1.svg]]

#+name: push-local
 #+begin_src plantuml :file images/git-push2.svg
<<push-start>>
...
#+RESULTS: push-local
[[file:images/git-push2.svg]]


 #+begin_src plantuml :file images/git-push3.svg
<<push-start>>
title after git push

DEV_GIT_COMMIT(2, 2)
DEV_GIT_COMMIT(3, 3)
DEV_GIT_COMMIT(4, "4 \n origin/master\nand\nlocal/master") #Magenta
...
#+RESULTS:
[[file:images/git-push3.svg]]

*** Just laptop
#+NAME: laptop-net
#+begin_src plantuml :file images/gh-lpatop.svg :eval no-export :results value file raw
<<init-uml>>
FA_LAPTOP(mattl, matt-laptop) #Green


#+end_src

#+RESULTS:
[[file:images/gh-lpatop.svg]]
*** Primitive network
#+NAME: degen-net
#+begin_src plantuml :file images/gh-degen-network.svg :eval no-export :results value file raw
<<laptop-net>>

FA_SERVER(mattr,github.com/matt) #Green
...
#+RESULTS: degen-net
[[file:images/gh-degen-network.svg]]

*** Simple Network
#+NAME: simple-net
#+begin_src plantuml :file images/gh-abbrev-network.svg :eval no-export :results raw
<<degen-net>>

FA_CLOUD(alicer,github.com/alice) #Magenta
FA_CLOUD(bobr,github.com/bob) #Turquoise
FA_CLOUD(kalsangr,github.com/kalsang) #Gold
...
#+RESULTS: simple-net
[[file:images/gh-abbrev-network.svg]]

#+RESULTS:
[[file:images/gh-abbrev-network.svg]]
*** Full Network
#+begin_src plantuml :file images/gh-full-network.svg :eval no-export :results raw
<<simple-net>>
FA_LAPTOP(bobl, bob-laptop) #Turquoise
FA_LAPTOP(alicel, alice-laptop) #Magenta
FA_LAPTOP(kalsangl, kalsang-laptop) #Gold


bobr --> bobl: Clone 
alicer <--> alicel: Clone 
kalsangr <--> kalsangl: Clone

#+end_src

#+RESULTS:
[[file:images/gh-full-network.svg]]

*** Pull

#+begin_src plantuml :file images/gh-pull.svg :eval no-export :results value file raw
<<init-uml>>
...
mattl <-- mattr: Pull New Changes
mattr <-- alicer: Merge Changes
mattl --> mattr: Push changes back  remote
mattl --> mattl: make small change
mattl --> mattl: make another change

#+end_src

#+RESULTS:
[[file:images/gh-pull.svg]]
** Git!
...
- [[https://github.com/DigitalHistory/history-please][Navigate to the History-Please repository]]
...
- Scroll down to the [[https://github.com/DigitalHistory/history-please#how-to-participate][contributing instructions]]
- Additional tips:
  - reference issues & pull requests by number, e.g. [[https://github.com/DigitalHistory/history-please/pull/58][#58]]. Do this whenever you create a pull that fixes an issue, especially.
...
#+begin_slideblock
#+ATTR_HTML: :style min-width: 15vw;
[[file:images/gh-lpatop.svg]]
#+end_slideblock

#+ATTR_REVEAL: :frag (appear) 
#+begin_slideblock
#+ATTR_HTML: :style min-width: 20vw;
[[file:images/gh-degen-network.svg]]
...
#+ATTR_REVEAL: :frag (appear) 
#+begin_slideblock
#+ATTR_HTML: :style  min-width: 45vw;; 
[[file:images/gh-branch.svg]]

#+end_slideblock

*** COMMENT Activity Branches
#+begin_src plantuml :file images/gh-branch.svg :eval no-export :results value file raw
skinparam defaultTextAlignment center
left to right direction
...
FA_SERVER(mattr,github.com/matt-master) #Green

FA_LAPTOP(mattl, matt-laptop-master) #Green

FA_CLOUD(alicer, github.com/alice-master) #Magenta
FA_CLOUD(aliceb, github.com/alice-patch123) #Magenta

mattl <--> mattr: Clone
mattr --> alicer: Fork
alicer --> aliceb: Branch
aliceb --> mattr: PR

#+end_src

#+RESULTS:
#+ATTR_HTML: :style  min-width:75vw; 
[[file:images/gh-branch.svg]]
...
*** Activity Commits
#+begin_src plantuml :file images/commits.svg :eval no-export :results raw
<<init-uml>>
...
#+ATTR_REVEAL: :frag (appear) 
#+begin_slideblock
https://blog.developer.atlassian.com/wp-content/uploads/dac-import/merge-conflict.png
...
#+begin_slideblock
#+ATTR_REVEAL: :frag (appear) 
[[./images/git-push0.svg]]
#+ATTR_REVEAL: :frag (appear) 
[[./images/git-push1.svg]]
#+end_slideblock

#+begin_slideblock
#+ATTR_REVEAL: :frag (appear) 
[[./images/git-push2.svg]]
#+ATTR_REVEAL: :frag (appear) 
[[./images/git-push3.svg]]
...
#+begin_slideblock
https://wac-cdn.atlassian.com/dam/jcr:0718ae20-0dc5-482c-a38c-25e4f1a1a3b0/02.svg
#+end_slideblock
#+ATTR_HTML: :style grid-row: 2/4;
#+begin_slideblock
#+ATTR_HTML: :style min-width: 20vw;
[[file:images/gh-pull.svg]]
...
#+begin_slideblock
https://wac-cdn.atlassian.com/dam/jcr:00d011ed-03dc-440f-afc5-9b13d5e14fbf/bubble%20diagram-01.svg
#+end_slideblock

** Assignment
The assignment is now up on [[https://classroom.github.com/a/VJGuHMB6][Github Classroom]], [[https://q.utoronto.ca/courses/123949/assignments/219086][Quercus]], and [[https://digitalhistory.github.io/assignment/][the course website]]. We'll discuss it, and further setup questions, on Thursday.

* 02-02: More Git, Some Markdown, a Tiny Bit of HTML
:PROPERTIES:
:EXPORT_FILE_NAME: 02-02-more-git
:EXPORT_TITLE: 2-2: More Git, Some Markdown, a Tiny Bit of HTML 
:CUSTOM_ID: 02-02-git-markdown-html-2309
:END:
...
#+begin_slideblock
*what's underneath*
- these are the "real" repo of which the worktree is *just one partialrepresentation*. 
[[./images/repository.png]]
#+end_slideblock
...
:PROPERTIES:
:CUSTOM_ID: markdown-cheatsheet-cn24
:END:
- ~_emphasis~ and ~__strong__~ :: single "_" or "* "  surrounding words for /ital/, double for *bold*, triple for */bold ital/*
- > for blockquotes :: just preface your paragraph with an angle bracket and one space to get a quote block
- Separate paragraphs with an empty line :: (otherwise they'll run together) 
- headings start with one or more "#" and a space :: don't ofrget the space!! 
...
- install extension
- install npm
- follow [[https://classroom.github.com/a/VJGuHMB6][assingment link]]
- get clone link link
- start working.  
* 03-01: HTML: Machine-readable Text
:PROPERTIES:
:CUSTOM_ID: machine-readable
:EXPORT_FILE_NAME: 03-01-html-backlog
:END:
...
- ~<section></section>, <article></article>, <header></header> <footer></footer>~ :: also invisible-by-default "semantic" tags that create divisions in page
...
#+ATTR_REVEAL: :frag (appear) 
#+begin_slideblock
Found inside structural elements:
#+ATTR_REVEAL: :frag (appear) 
...
- *Clone  the excercse* [[https://github.com/titaniumbones/HabermasCode][at the Github Repository]]
#+BEGIN_NOTES
...
#+END_NOTES
** Assignment
[[https://digitalhistory.github.io/assignment/01-html-css/][The assignment is online]]. Let's talk about it.  

** COMMENT HTML to Markdown
:PROPERTIES:
...
sroot.appendChild(createElementFromHTML(htmlString));
</script>
#+end_export
...
:PROPERTIES:
:EXPORT_FILE_NAME: 04-01-web-signifies
:END:
** Today 
- Assignment 0 review
...
- watch me work briefly with [[https://github.com/DigitalHistory/css-layout.git][css-layout practice repo]]

* 04-02, CSS Part 2: Layout
:PROPERTIES:
:EXPORT_FILE_NAME: 04-02-css-layout
:CUSTOM_ID: 04-02-css-part-2-layout-494e
:END:
...
:PROPERTIES:
:reveal_extra_attr: class="twoc"
:CUSTOM_ID: default-view-(just-some-silly-styling)-3cbb
:END:
#+begin_slideblock
#+ATTR_REVEAL: :no-klipsify t
#+NAME: basic_layout
...
#+CALL: generate_shadow_dom[:results html :exports "results"]("basic_layout", basic_layout) 
** Display: None
:PROPERTIES:
...
#+CALL: generate_shadow_dom[:results html :exports "results"]("display_none", none_layout) 
** Display: Flex
:PROPERTIES:
...
#+CALL: generate_shadow_dom[:results html :exports "results"]("display_flex", flex_layout)
...
#+CALL: generate_shadow_dom[:results html :exports "results"]("display_grid", grid_layout)
...
#+Attr_reveal: :No-Klipsify T
#+NAME: generate_shadow_dom
#+begin_src inline-js :var elId="shadowEx"  :var dom_block=css_rules0 :exports none
function createElementFromHTML(htmlString) {
...
:header-args: :tangle Exercises/js-lecture-code-snippets/lec-05-01-js-intro.js :noweb yes
:ORG_LMS_ANNOUNCEMENT_ID: 406091
:ORG_LMS_ANNOUNCEMENT_URL: https://q.utoronto.ca/courses/123949/discussion_topics/406091
:ORG_LMS_POSTED_AT: 2019-10-08T17:47:11Z
:END:
...
*** 
Pull in changes to [[https://github.com/DigitalHistory/lecture-code-snippest][the code snippets repo]]
...
*** using JSON data
#+begin_src js :var orgs=rev-orgs :results value
<<rev-orgs>> 
...
:PROPERTIES:
:EXPORT_FILE_NAME: 05-02-distant-reading
:EXPORT_TITLE: Distant Reading
:END:
** COMMENT Notes
:PROPERTIES:
...
:PROPERTIES:
:EXPORT_FILE_NAME: 06-02-text-as-data
:CUSTOM_ID: 06-02-text-as-data-f1aa
:END:
...
* COMMENT 04: Pt 2, Distant Reading 1 (2019-01 version)                                    
:PROPERTIES:
...
* COMMENT 07-02 -- just used the exercise repo for that!!
* 08-01: Homework Help Day
:PROPERTIES:
:EXPORT_FILE_NAME: 08-01-homework-help
:header-args: :noweb yes
:END:
** Today
- [[https://github.com/DigitalHistory/dom-exercises][working again from this repository]]
...
#+begin_slideblock
[[./images/e2019/globe-results-2019.png]]
#+end_slideblock

#+begin_slideblock
[[./images/e2019/cbc-results-2019.png]]
#+end_slideblock

#+begin_slideblock
[[./images/e2019/star-results-2019.png]]
#+end_slideblock
...
<card class="card">
  ...
  <footer>
    <a href="https://digitalhistory.github.io/">A Project of HIS393</a>
  </footer>
</card>
#+end_src
...
* COMMENT old javascript lecture notes -- contains some important araay & string methods
:PROPERTIES:
:CUSTOM_ID: javascript-30da
:END:
** For loops!
...
* COMMENT 02: What the web signifies | HTML & CSS                  
:PROPERTIES:
...
* COMMENT 06: Distant Reading, Catchup, yet more javascript
:PROPERTIES:
...
:PROPERTIES:
:EXPORT_FILE_NAME: 08-02-why-spatial-history
:CUSTOM_ID: 07:-why-spatial-history?-c5b0
:END:
** Assignment Notes
- Assignment 3: good luck!!
- Assignment 4: not up yet! But the *non-technical instructions* in [[https://github.com/DigitalHistory/advanced-topics/tree/master/spatial-history#assignment][last year's version]] will still apply. There will, however, be some significant differences in the technical content. 
...
https://ncrc.org/wp-content/uploads/2018/09/PHILLY_HOLC-2.jpg
#+end_slideblock

#+ATTR_REVEAL: :frag appear
#+begin_slideblock
@@html:<iframe width="100%" height="800px" src="https://dsl.richmond.edu/panorama/redlining/#loc=12/40.017/-75.205&city=philadelphia-pa" allowfullscreen></iframe>@@
#+end_slideblock


** Questions for us (again)
:PROPERTIES:
:CUSTOM_ID: questions-for-us-3e234
:END:
- What does it mean to foreground spatial characteristics in a /spatial/ history?
- How do maps & media /transform/ the kinds of stories we can tell
* 09-01: Spatial History with Leaflet Maps
:PROPERTIES:
:CUSTOM_ID: spatial-history-with-leaflet-maps-a3dc
:EXPORT_FILE_NAME: 09-01-spatial-history-leaflet
:END:  
** COMMENT Cats
#+ATTR_HTML: :class paired
[[./images/cat.jpg]]
...
:PROPERTIES:
:CUSTOM_ID: leaflet-maps-and-spatial-history-eeb1
:END:
...
:PROPERTIES:
:CUSTOM_ID: controlling-leaflet-maps-a206
:END:
...
#+BEGIN_ASIDE
An *API* is an "Application Programming Interface": a communications channel that lets programs talk to each other.  By "loading" the Leaflet API, our web pages can communicate directly with Leaflet's servers to modify the map that Leaflet is presenting to us.  In fact, most of the interesting stuff happening on the web these days happens via these machine-to-machine communication channels.  Cf. [[https://www.youtube.com/watch?v=s7wmiS2mSXY][this helpful video]] or [[https://en.wikipedia.org/wiki/Application_programming_interface][Wikipedia]] for more API info.
#+END_ASIDE

#+BEGIN_NOTES 
For our exercise today, we are really only interested in two layers:  the "basemap", that is, the street or satellite map most of us use on an almost-daily basis; and the /marker layer/, in which all of the little pins on a map are stored.  We access these layers, and create those markers, with Javascript, by making /calls/ to the /Leaflet Maps API/.
...
:END:
[[https://classroom.github.com/a/Irv1gDIe][online here]]
** The code
:PROPERTIES:
...
* 09-02: More GIS
:PROPERTIES:
:EXPORT_FILE_NAME: 09-02-more-gis
:END:
...
# /home/matt/HackingHistory/images/x\nytimes-no-css.png http://digitalhistory.github.io/wp-content/uploads/2017/01/wpid-nytimes-no-css.png
